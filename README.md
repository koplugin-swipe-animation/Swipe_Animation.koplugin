...
3. 将您下载解压后的文件夹根目录中的 `frontend` 和 `patches` 文件夹，**复制** 并**覆盖**到设备上的 `koreader` 目录中（**注意**：请勿删除原文件夹）。本补丁不覆盖 `ffi/` 目录中的任何系统文件。
   - 常见路径：`D:\.adds\koreader\`
   - **特别说明**：如果您的设备原本就支持原生翻页动画，且您不希望使用本补丁的软件擦除动画，而仅希望为 PDF 文件开启设备原生的硬件级翻页动画，您可以选择**只将 `patches` 文件夹里的 `2-pdf-animation.lua`**复制到设备上的 `koreader/patches/` 目录下即可（KOReader 2026.07.1）。
